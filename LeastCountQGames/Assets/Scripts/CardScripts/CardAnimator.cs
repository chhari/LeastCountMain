...
namespace QGAMES
{
    public class CardAnimation
    {
        ...
        public CardAnimation(Card c, Vector2 pos, Vector3 scl)
        {
            card = c;
            destination = pos;
            rotation = Quaternion.identity;
    ...
    public class CardAnimator : MonoBehaviour
    {
        ...
        public void DealDisplayingCardsToLocalPlayer(MyPlayer player, int numberOfCard)
        {
            ...
            for (int i = start; i > finish; i--)
            {
                ...
                player.ReceiveDisplayingCard(card);
                cardsToRemoveFromDeck.Add(card);
                AddCardAnimation(card, player.NextCardPosition(),new Vector3(0.2f,0.2f,0));
            }
        ...
        public void AddCardAnimation(Card card, Vector2 position, Vector3 scale)
        {
            CardAnimation ca = new CardAnimation(card, position, scale);
            cardAnimations.Enqueue(ca);
            working = true;
        }
